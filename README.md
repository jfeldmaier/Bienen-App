...
| `migrate.py` | A script to handle database migrations using Flask-Migrate. |
...
| `var/` | Contains variable data, such as the SQLite database (`bienen.db`) and uploaded files. |
